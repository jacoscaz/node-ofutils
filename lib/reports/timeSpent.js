...
const timeSpentRegexp = /^\s*(?:time|time-)?spent\s*(\d{4}-\d{2}-\d{2})\s*(\d+)([hmd])\s*([^\s].*?)?\s*$/;
  ...
  projects.forEach((project) => {
    project.tasks.forEach((task) => {
      ...
      task.note.split(/\r?\n/).forEach((line) => {
        ...
        if (!match) return;
        const [, date, qty, unit, description] = match;
        ...
        opts.groupBy.forEach((criteria) => {
          switch (criteria) {
            ...
            case 'task':
              ...
              break;
            case 'worklog':
              const worklogId = task.id + worklogSerial++;
              if (!currentGroups[worklogId]) {
                currentGroups[worklogId] = {
                  date,
                  description,
                  total: 0,
                  groups: {},
              ...
              currentGroups[worklogId].total += hours;
              currentGroups = currentGroups[worklogId].groups;
              break;
            default:
